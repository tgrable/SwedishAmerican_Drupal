...
/**
 * Provides a 'twitter Post Feed' Block.
 *
 * @Block(
 *   id = "twitter_post_feed",
 *   admin_label = @Translation("twitter Post Feed"),
 * )
 */
...
define('CONSUMER_SECRET', 'mVm9oSGIeQf1RjCJxsQpgkxUr9DxumJYHICvSRQXqx1QDTOylZ');
 ...
 class TWPostFeedBlock extends BlockBase {
  ...
  private function get_twitter_data() {
    ...
    // lets run a search.
    $bearer_token = $this->get_bearer_token(); // get the bearer token
  ...
  */
  function search_for_a_term($bearer_token, $query, $result_type='mixed', $count='15'){
    $url = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=SAHealthSystem&count=1&tweet_mode=extended"; // base url
    ...
    $headers = array( 
      ...
      "Host: api.twitter.com", 
      "User-Agent: Swedish American Application-only OAuth App v.1",
